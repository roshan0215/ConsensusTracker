...
export default function SignIn() {
  ...
  const login = async () => {
    ...
    setError(null);
    try {
      const out = await apiFetch("/api/auth/login", { method: "POST", body: { email, password }, auth: false });
  ...
  const requestLink = async () => {
    ...
    setMessage(null);
    try {
      await apiFetch("/api/auth/request-magic-link", { method: "POST", body: { email }, auth: false });
      setMessage("If that email exists, a sign-in link was sent.");
    } catch (e) {
      setError(String(e.message || e));
    } finally {
      setBusy(false);
    ...
    <Layout variant="centered">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="mt-3 text-xl font-semibold text-slate-900">Welcome back</h1>
        ...
        <div className="card p-8">
          ...
          <div className="space-y-4">
            ...
            <button className="btn-primary w-full py-3" disabled={busy} onClick={login}>
              {busy ? "Signing in..." : "Sign in"}
            </button>
          </div>

          <div className="mt-6 pt-6 border-t border-slate-200">
            <p className="text-xs font-medium text-slate-600 mb-3">Or sign in with a magic link</p>
            <button className="btn-secondary w-full" disabled={busy || !email} onClick={requestLink}>
        ...
        <p className="mt-5 text-center text-sm text-slate-500">
          ...
          <a className="font-medium text-emerald-600 hover:text-emerald-700" href="/register">Create one for free</a>
        </p>
      </div>
    </Layout>
  );
